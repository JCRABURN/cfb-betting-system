...
## 26. External review, disposition (2026-08-04)

An outside review of the system produced a list of proposed changes, ranging from operational hardening to a modeling roadmap. Each item was evaluated independently rather than accepted or rejected as a package. **Accepted: items 1-6 below**, all implemented and tested this session. **Declined**, with reasoning: the modeling roadmap, a Postgres/Parquet migration, a package restructure, byte-for-byte card reproducibility, and a 12-code failure taxonomy — see "Declined" below. One item (a pool-ranking gap) was accepted in substance but implemented this project's own way rather than as proposed.

### Accepted and implemented

**1. Raw API response archival.** New `raw_payloads` table (`db.py`): every CFBD and Odds API request now archives its raw response body (gzip-compressed, sha256-checksummed) before parsing, tagged with `provider`, `endpoint`, redacted `request_params`, `http_status`, `parser_version`, and `rows_accepted`/`rows_rejected` (filled in after parsing succeeds). Wired into all 5 of `fetch_stats.py`'s CFBD calls, `fetch_odds.py`'s Odds API call, and — beyond the two files named in the request, since "every API request" is the literal scope — `post_game_audit.py`'s own direct `/games` call for final scores. Purpose: when the next parser bug surfaces (every historical ingestion path in this project has had one — §24's three bugs, found only by inspecting real data after the fact), the stored response can be replayed through a fixed parser instead of re-spending API calls (the Odds API's free tier is capped at 500/month) and guessing which rows a bad parse polluted.

Secret redaction (`db._redact_params`) is a property of `archive_raw_payload()` itself, not something every call site has to remember: any params key matching `key`/`token`/`secret`/`password`/`authorization` (case-insensitive) is replaced with `<redacted>` before storage. Verified live against the real Odds API call, where `apiKey` genuinely lives in query params (CFBD's key lives in a header, so there's nothing to redact there, but the mechanism doesn't need to know that case by case).

**Weekly `cfb.db` growth estimate**, from real, live-measured gzip byte sizes (not the artificially-empty 2026 preseason SP+/EPA/records — those were measured against the real, populated 2025 season instead, since using today's empty preseason numbers would understate the honest cost):

| Endpoint | gzip size | Called by |
|---|---|---|
| `/calendar` | 396 B | every `get_current_week()`/`get_week_date_range()` call |
| `/games` (weekly slate, ~99 games) | 5,665 B | `fetch_stats.py`, `post_game_audit.py` |
| `/ratings/sp` (full season, 137 teams) | 4,226 B | `fetch_stats.py` |
| `/stats/season/advanced` (full season, 136 teams) | 85,128 B | `fetch_stats.py` |
| `/records` (668 rows) | 25,129 B | `fetch_stats.py` |
| Odds API `/odds` (126 games) | 9,491 B | `fetch_odds.py` |

Per-run totals: **Tuesday** (`weekly_report.yml`: `fetch_stats.py`'s 5 calls + `fetch_odds.py`'s 3 calls) ≈ 130.8 KB; **Thursday** and **Saturday** (`midweek_line_pull.yml`: `fetch_odds.py`'s 3 calls each) ≈ 10.3 KB each; **Monday** (`post_game_audit.py`'s 2 calls) ≈ 6.1 KB. **Weekly total ≈ 157 KB**, plus roughly 4 KB of per-row overhead (checksums, params, timestamps) across the week's ~16 archived calls — call it **~160 KB/week**. Over a 15-week regular season, that's **~2.4 MB added to `cfb.db`** — a small, cheap addition relative to the DB's existing size, dominated almost entirely by `/stats/season/advanced`'s 85 KB (the full-season EPA/success-rate/havoc-rate table, refetched in full every Tuesday).

**2. Immutable locked contest lines.** New `contest_entries` table: the source of truth for pool lines, `UNIQUE(contest, season, week, game_id)`, inserted once via `pool_view.ingest_contest_csv()` (`INSERT OR IGNORE` — re-ingesting the same CSV is idempotent, never a silent overwrite). The weekly CSV (`data/pool_picks/week_{week}_{season}.csv`) stays the entry mechanism, but it now ingests INTO this table rather than being read directly; `build_pool_view()` reads FROM `contest_entries` via `load_pool_entries_from_db()`, both in `pool_view.main()` and `build_dashboard.py`. Raw team names are stored exactly as typed; normalized names resolve via `fetch_odds.resolve_school_name()` (the same resolution the live odds feed already uses), including `picked_side`.

The only sanctioned way to change an already-locked row is `pool_view.correct_contest_entry(conn, entry_id, reason, new_locked_home_spread=None, new_picked_side=None)`: it writes the ORIGINAL `locked_home_spread`/`picked_side` to a new `contest_entry_corrections` ledger row BEFORE updating `contest_entries` in place, and requires a non-empty `reason` and at least one actual new value. `tests/test_pool_view.py::test_correct_contest_entry_preserves_original_values` is the explicitly required proof: it asserts the correction ledger row carries the pre-correction values, not the post-correction ones. Re-ingesting a CSV for an already-locked `(contest, season, week, game_id)` is verified live to refuse the new value outright, not overwrite: `test_reingesting_a_changed_csv_does_not_alter_the_locked_spread` proves the row's spread/side/`corrected_at` are untouched even when the CSV now disagrees with them. `ingest_contest_csv()` returns `{"inserted": N, "skipped": [...]}` rather than a bare count (follow-up, accepted 2026-08-05) — `pool_view.main()` prints the skipped games by name ("N row(s) already locked, not modified — use correct_contest_entry() to change a locked line: ..."), so a re-run after fixing a CSV typo makes it obvious the fix didn't silently apply, rather than reporting an unremarkable `inserted=0`.

**3. Test gate.** All three workflows (`weekly_report.yml`, `midweek_line_pull.yml`, `post_game_audit.yml`) now run `python -m pytest -q` (step id `tests`) immediately after `pip install -r requirements.txt` — before any fetch/generate/commit step. Every step that touches `data/cfb.db` explicitly checks `steps.tests.outcome == 'success'` in its own `if:` condition, including steps that also carry `if: always()` for their own, separate resilience reasons (so a test failure can't be masked by that `always()`). `requirements.txt` already listed `pytest`; the workflows previously only ran `pip install requests`, so pytest was never actually available in CI before this.

**4. Concurrency.** All three workflows share one `concurrency: { group: cfb-betting-system-db, cancel-in-progress: false }` block — identical group name across all three files, so they serialize against EACH OTHER (not just against themselves), queuing a second trigger instead of running two mutations of the same committed DB file at once, which is what caused the push races this closes.

**5. SQLite hardening (git-as-DB kept, Postgres declined — see below).** `db.integrity_check()` runs `PRAGMA integrity_check` and hard-fails (`sys.exit(1)`, no `|| true`) if the result isn't exactly `['ok']`; wired into every workflow as its own step (id `integrity`), positioned after the generate/build steps and before the commit step, which additionally requires `steps.integrity.outcome == 'success'`. A pre-mutation backup of `data/cfb.db` uploads as a workflow artifact (`actions/upload-artifact@v4`, 14-day retention) immediately after checkout, before any fetch step can touch the file. The commit step does a bounded `git pull --rebase origin main` / `git push` retry loop (5 attempts) instead of a bare push.

**6. Pool ranking helper.** `pool_view.rank_pool_picks(pool_view, card=None)`: ranks the pool's own locked picks (not model recommendations) by drift confirmation — `signed_drift_vs_pick` from `build_pool_view()`, i.e. has the market moved toward this pick since it was locked — as the PRIMARY sort key, with model edge from `card_generator.build_card()` as a TIEBREAKER ONLY when drift ties. A pool pick whose matching card game is flagged `low_confidence_large_edge`, `low_confidence_prior_season_data`, or `no_pick_extrapolation` is excluded entirely, not just denied its tiebreaker — those are exactly the model's least-reliable slices (§19-20/§25). A pool pick with no matching card game at all stays eligible, ranked on drift alone. Returns up to 5 entries. Surfaced in `build_dashboard.py` as a new "Ranked Pool Picks" panel, right under Pool Drift.

### The one accepted-but-not-as-proposed item

The review's pool-ranking suggestion was accepted in substance — surfacing which locked pool picks are strongest is a legitimate gap — but implemented this project's own way rather than as proposed: drift confirmation (an observed market signal) as the primary ranking input, with the EPA-only model's edge admitted only as a tiebreaker behind it, never as a co-equal or primary signal. This follows directly from §19-20's finding that the model has no demonstrated standalone edge over the market.

### Declined

**GBM/ensemble/state-space modeling roadmap, paired with a softened acceptance bar.** The review proposed both more model flexibility (gradient-boosted trees, ensembling, state-space formulations) AND a lower bar for accepting a candidate feature/model, at the same time. On 5 seasons of data (~3.5k graded games), more tunable knobs plus a lower acceptance bar is a textbook overfitting setup — it would make it easier to publish a result that looks like an edge and is actually noise fit to the exact backtest window, which is the one failure mode §6/§14/§19-20's entire testing discipline exists to prevent. Declined as a package. Two narrower pieces of it are kept as candidates for FUTURE modeling work, contingent on clearing the SAME pre-registered acceptance bar as every other feature tested so far, not a relaxed one:
- A market-residual target formulation (predicting the deviation from the closing line rather than the raw margin) — plausible in principle, untested here.
- A lockbox holdout (a slice of games never touched during any iteration of model development, reserved for a single final check) — good practice in general, but not a substitute for the pre-registered bar itself, and not implemented this session.

**Postgres/Parquet migration.** Declined — git-as-DB (`cfb.db` committed directly) stays. Item 5's SQLite hardening (integrity check, pre-mutation backup artifact, rebase-retry push) addresses the actual failure modes a migration would have solved (corruption risk, push races, no rollback point) without taking on a hosted database, a second piece of infrastructure to maintain, or a rewrite of every script's `db.get_connection()` call. This project's budget and owner-context (personal decision-support tool, ~$10-30/month, single maintainer) don't support the added operational surface.

**Package restructure.** Declined — no functional problem identified that a restructure would fix; the flat `data/`/`models/` layout with `sys.path.insert` has worked without incident across 25+ implementation phases.

**Byte-for-byte card reproducibility.** Declined — `card_generator.py`'s output already depends on the live betting line at generation time by design (that's the whole point of comparing against a "consensus" or "current" line); byte-for-byte reproducibility would require freezing that input too, which conflicts with the tool's actual purpose (a live decision-support read, not a deterministic report generator).

**12-code failure taxonomy.** Declined — the existing `confidence` taxonomy (`standard` / `low_confidence_large_edge` / `low_confidence_prior_season_data` / `no_pick_extrapolation`, §25) already covers every distinction that's mattered in practice across three real ingestion bugs found by inspecting live data (§24). Twelve codes for failure modes not yet observed is speculative categorization ahead of a demonstrated need — this project's engineering rules explicitly discourage building for hypothetical future requirements.

### Tests

27 new tests: 12 in `tests/test_pool_view.py` (contest_entries insert/idempotency/name-normalization, `load_pool_entries_from_db`, the required correction-preserves-original-values proof, correction validation, `rank_pool_picks` drift-primary/edge-tiebreak/low-confidence-exclusion/no-card-match/cap-at-5), 2 in `tests/test_build_dashboard.py` (ranked-picks empty state, rendered row), 6 in `tests/test_db_schema.py` (new tables exist, `integrity_check()` on a healthy DB, `archive_raw_payload` round-trip + checksum, `update_raw_payload_counts`, secret redaction, `contest_entries` UNIQUE constraint), and 7 in `tests/test_workflows.py` (shared concurrency group across all three workflow files, test gate precedes every mutation step, mutation steps gated on test+integrity outcome, integrity check precedes commit, backup artifact precedes every mutation step, bounded rebase-retry present, `requirements.txt`-based install). 252 tests pass across the whole suite.

## 27. NFL scope, Phase 1: data ingestion (2026-08-24)

New architecture decision: **two separate models, not one shared one.** College and NFL get separate stats tables (`nfl_` prefix) and separate future backtest/model code, but share plumbing that's genuinely sport-agnostic (drift views, the post-game audit, the dashboard). This section is data ingestion only — backfilling schedules, historical lines, and point-in-time stats. The backtest harness itself is explicitly next session's work (§ near the end of this entry), not started here.

### Feasibility gate, checked before writing anything

An NFL backtest needs genuine opening-line coverage the way the college side did (§14: 2020 was a total loss there, zero opening lines archived at all). Checked three things live before committing to build:

1. **nflverse/nfldata's `games.csv`** has a `spread_line` column with 100% coverage 1999-2025 — but nflverse's own data dictionary doesn't say whether it's opening or closing. Empirically resolved (not guessed): cross-referenced it against nflverse's own labeled 2021 openers (`initial_lines.csv`, the only season that file covers) for real games -- every one differed from `spread_line` by a realistic in-week movement amount. `spread_line` is a closing line.
2. **sportsbookreviewsonline.com's NFL archive** genuinely has real, independently-verifiable opening lines — confirmed by cross-checking its `Open` column against the same 2021 nflverse openers (both track the same real market, from different vendors, expected small book-to-book differences, never matching the close instead). Discontinued January 16, 2023 (confirmed from the site's own text, via a Wayback snapshot), last season 2022-23.
3. **nflverse's play-by-play** is raw, not a vendor rating — confirmed `epa`/`success` are 100% populated even in the earliest (1999) season file. This is NOT the SP+ problem (§13: CFBD's SP+ has zero historical granularity, nothing to backfill). Any cumulative-through-week-N cut is an aggregation this project computes itself from raw plays.

Verdict: feasible, with one real constraint (below).

### What was built

- **`data/nfl_teams.py`** — nflverse's 35 real team codes (32 current + 3 retired: STL/SD/OAK for the Rams/Chargers/Raiders relocations during this window) and two name-resolution tables: SBR's archive (season-aware for the 3 relocated franchises; the archive itself spells team names two different ways across its own history — "Kansas City" in one era's files, "KansasCity" in another — resolved by normalizing spaces/periods out of both sides before matching, not by listing every spelling twice) and the Odds API's standard "City Nickname" convention (unverified for NFL specifically — see the flag below).
- **`db.py`** — `nfl_games` (TEXT primary key; nflverse's own string ids like `2024_01_ARI_BUF` can't share `games`' INTEGER primary key, which is CFBD's numeric id space — this alone would force a separate table even without the two-model decision) and `nfl_team_stats` (mirrors `team_game_stats`' point-in-time shape). `betting_lines` gets a `league` column (`'cfb'` default, migrated onto the real production table) instead of a separate table — the row shape is genuinely sport-agnostic and this is where "shared plumbing" actually applies. `test_no_mixed_league_rows` plus a second test proving `games`/`nfl_games` can't cross-resolve even by accident (different id types, not just different tables).
- **`data/backfill_nfl_games.py`** — nflverse's `games.csv`, one small public file (no auth, no rate limit), populates `nfl_games` and writes closing-line rows for completed games only (a not-yet-played game's `spread_line` is just the current number, not a genuine close). **A real sign bug found and fixed here**: nflverse's own convention is positive=home-favored, the opposite of this project's `home_spread` convention (negative=home-favored) used everywhere else. Caught by the exact opener/closer verification requested — the real Dallas @ Tampa Bay 2021 game showed `+10.0` from this path and `-10.0` from the SBR path for the same real closing number before the fix.
- **`data/backfill_nfl_historical_lines.py`** — ingests the 15 committed archive files. The real parsing problem, found by testing against actual data instead of assuming a format: each game is two rows (one per team), and which row's `Open`/`Close` cell holds the spread vs. the total is **not** fixed per row — it can flip between Open and Close within the same game, because it's really tracking which team is favored, and the favorite itself can flip mid-week. A per-row model failed on ~8% of real games; the corrected per-cell model (classify Open and Close independently, exactly one of the two rows must be spread-shaped and the other total-shaped) resolves 4,018 of 4,025 real games (99.83%). The 7 that don't are reported by name and reason, not guessed. A second real bug caught during this build: the two classification thresholds were set to overlap instead of leaving a genuine gap, which (due to if/elif order) silently made one of them dead code — fixed to a real gap, re-validated against all 4,025 games, cost exactly one previously-resolved game.
- **`data/backfill_nfl_pbp_stats.py`** — nflverse's play-by-play (`nflverse-data` GitHub Releases, `.csv.gz`, stdlib `gzip`+`csv`, no new dependency), aggregated locally into cumulative-through-week-N EPA/success rate per team, same lookahead discipline as `get_team_stats_as_of()`. Verified against Detroit's real 2024 season: offense EPA/play climbs from +0.106 (week 1) to +0.180 (week 17), defense improves through midseason then fades late, and weeks 4/5 are identical — their real bye week, correctly carried forward rather than silently skipped.
- **`data/fetch_nfl_odds.py`** — live NFL odds via the existing Odds API key (`americanfootball_nfl`), written into `betting_lines` with `league='nfl'`. Built and unit-tested; **not live-verified** — see the flag below.
- **`openpyxl`** added to `requirements.txt` — the committed archive is real `.xlsx`, not CSV; a stdlib-only path means hand-writing an OOXML zip+XML reader (shared-strings table, cell-ref parsing, type coercion), a well-known undertaking this small, stable, pure-Python library already solves. Read-only usage here, no write-back.

### Recorded, not fixed (per instruction)

**1. The NFL backtest window is 2007-2021 (15 seasons), not 2007-2022.** sportsbookreviewsonline.com's own final season (2022-23) was never crawled by the Wayback Machine — confirmed via its CDX API returning zero snapshots for that specific file, while every other season's file and the index page itself were captured. The index page linking to the 2022-23 file was crawled; the file wasn't. This is permanent and unrecoverable — the source site is dead (discontinued Jan 16, 2023), so no later crawl will ever happen. 15 seasons still clears the "5+ seasons of verified real openers" bar from a wide margin.

**2. `fetch_nfl_odds.py` is unverified against a live response and must be gated before NFL data is trusted.** `api.the-odds-api.com` was unreachable from the build environment (TLS connection reset) while CFBD worked fine from the same machine moments apart — the same network-level block already documented for the CFB side (§11), not something fixable from inside the build. The script's team-name mapping (`nfl_teams.ODDS_API_NFL_TEAM_TO_CODE`) is built on the Odds API's standard "City Nickname" convention, confirmed correct for every other sport this project integrates, but not confirmed for NFL specifically. This matters more than it might otherwise: team-name resolution has already produced two real, live-data-only-discovered bugs on the CFB side (§11's mascot-suffix mismatch, §24's consensus-masking bug) — assumed-correct name mapping has a real track record of being wrong in exactly this system. **Required before any NFL live-odds data is trusted**: run `fetch_nfl_odds.py` for real once network access allows it, and check the actual response's team-name strings against the mapping before relying on a single row it writes.

### Next session: the NFL backtest harness

Same standard as the college side, not a looser one: walk-forward (train on seasons < Y, test season Y), opening-line input only (never substitute a later line), point-in-time stats enforced structurally (a sealed feature package, no database handle inside the prediction function — the three guarantees from §14, unchanged). Adversarial tests (planting a leak, confirming the harness refuses it) before any baseline model runs, per the same discipline §14 used.

**Reuse `backtest_harness.py`'s machinery where the shape is genuinely the same, rather than forking a parallel copy that can drift out of sync.** `fit_linear`/`fit_multilinear`, `grade_ats`, `unit_pl`, `calculate_clv`, and `mcnemar_test` are pure math with no CFB-specific assumption baked in — these should take an NFL feature package and work unmodified. What's genuinely sport-specific and needs an NFL-side counterpart: `get_team_stats_as_of()` (reads `team_game_stats`; NFL needs the same function shape reading `nfl_team_stats`), `get_opening_line()`/`get_closing_line()` (reads `betting_lines` unscoped by league right now — needs a `league='nfl'` filter added, a decision for next session on whether that's a parameter on the existing functions or separate ones), and `list_games()`/`get_pregame_stats()` (reads `games`; NFL needs `nfl_games`). Decide explicitly next session whether the shared functions take a `league` parameter or whether NFL gets its own thin wrapper module calling into the same core math — don't default to copy-pasting the whole file.

## 28. Week 1 close-out: alias gap, pool-ranking gap, straggler-week gap (2026-09-03 to 2026-09-08)

Three real bugs found and fixed while running the system against real Week 1 2026 data end-to-end (a SplashSports pool slate ingestion, a live dashboard screenshot, and the season's first post-game audit) — none guessed, all confirmed against real data or real production code before fixing.

**1. Missing team-name alias: "Louisiana-Monroe" → "UL Monroe".** Ingesting the user's real 43-game Week 1 pool slate, 1 of 43 games failed to resolve against `games` — CFBD's real name is "UL Monroe," not the pool sheet's "Louisiana-Monroe." Same alias-gap class as the existing App State/UMass/Southern Miss entries in `fetch_odds.KNOWN_TEAM_ALIASES` (§11) — added `Louisiana-Monroe`/`Louisiana Monroe`/`UL-Monroe` → `UL Monroe`, regression-tested.

**2. `card_generator.py`'s presentation-only suppression was being mis-copied into ad hoc reports as if it meant "the model has no pick here."** Per §25, `no_pick_extrapolation` only suppresses the PRESENTED number — `side`/`edge` are always computed and retained. A first pass at the pool slate's model-read CSV left `model_side`/`model_edge` blank for those 26 games, incorrectly implying the model produced no opinion. Corrected to show every game's actual pick, labeled `recommended_low_confidence` / `not_recommended_extrapolation` instead of omitted.

**3. `pool_view.rank_pool_picks()` excluded `low_confidence_prior_season_data` outright — the same flag every Week 1 game carries by construction — so the Ranked Pool Picks dashboard board could never show anything for the entirety of Week 1, every season.** This was inconsistent with #2's own fix and with how the Model Picks board already treats the same flag (a real, displayed pick, not a suppression state). `POOL_RANKING_LOW_CONFIDENCE_FLAGS` renamed to `POOL_RANKING_EXCLUDED_FLAGS`, now only the two genuine no-signal states (`low_confidence_large_edge`, `no_pick_extrapolation`). Verified against real data: the Auburn/Baylor pool entry (edge 0.73) now surfaces on the live dashboard, badged "low conf.," where it was previously excluded.

**4. `post_game_audit.py`'s `main()` trusted `get_current_week()` alone to decide what to grade — a real gap for any week whose last kickoff lands after the Monday 11:00 UTC audit cron.** Confirmed live: Week 1's Florida State @ SMU game kicked off Monday 23:30 UTC, well after that week's audit had already run (16:18 UTC, itself a few hours late per the cron-is-best-effort finding in §22). By the time the audit could run again, `get_current_week()` had already rolled over to Week 2, and a "grade whatever week is current" run would have looked for Week 2 picks (none existed yet) and silently never come back for the Week 1 straggler — it would have sat `pending` forever without manual intervention. Fixed: new `find_weeks_with_pending_picks(conn, season)` returns every week with an outstanding pending pick; `main()` unions that with the current week and grades each, rather than grading only the current week. Verified live: this caught and correctly graded the dangling FSU/SMU pick (Florida State favored by 1.5, SMU won outright 27-24 — a loss, CLV -1.3) that a plain current-week-only run would have missed entirely.

**5. `card_generator.persist_picks_to_db()` never persisted a `picks` row at all for `no_pick_extrapolation` games — meaning their real-world outcome was tracked nowhere, ever, contradicting the explicit design goal that the model picks every game (so it can be checked over time), only the PRESENTATION calls a suppressed game "not recommended."** Confirmed live: only 21 of 49 real Week 1 2026 lined games had a trackable `picks` row before this fix. Fixed: every game now gets a persisted row; `qualifies` (an INTEGER column inherited from the old pre-EPA-only weighted model, previously always hardcoded 0 and otherwise unused) now marks a genuine recommendation (1) vs. a suppressed no-pick (0). `post_game_audit.py` grades every row regardless of `qualifies`; `build_season_ledger()` filters to `qualifies = 1` so the headline ATS/ROI record is exactly what it always was. A `persist_picks_to_db()` re-run now UPDATEs an existing PENDING row in place if its numbers or qualifies changed (generalizing the old "retract on becoming no_pick_extrapolation" special case to every game), rather than ever deleting a row — a settled row is still never touched. One-time data correction for the 21 pre-existing rows (all real recommendations under the old code, since a no_pick_extrapolation game never had a row before this fix) via a new, permanently-idempotent `db._backfill_qualifies_column()`, keyed structurally (a `qualifies=0` row with no `"no_pick_extrapolation"` in `confidence_signals` must predate this fix) rather than a point-in-time cutoff, so it's safe to leave running on every `init_db()` call rather than a one-shot script. Verified live: backfilling Week 1 added 28 more graded rows (27 no-picks + 1 previously-missed qualifying game); the qualifying Ledger stayed at its correct 22-pick record (8-14-0, -30.6% ROI) while the newly-tracked no-pick bucket went 13-14 on its first tracked week — a first real data point for whether the suppression rule holds up going forward, not a conclusion from n=27.

All five fixed and tested (regression tests added in `test_fetch_odds.py`, `test_pool_view.py`, `test_build_dashboard.py`, `test_post_game_audit.py`, `test_card_generator.py`, `test_db_schema.py`); full suite green (347 tests) before commit.

## 29. Week 2 card came back empty: point-in-time stats had no live-automation caller at all (2026-09-09)

Ingesting the user's real Week 2 2026 SplashSports slate (49 games, plain CSV this time, not xlsx) and running the card against it surfaced a severe gap: `card_generator.build_card(conn, 2026, 2)` returned **0 of 49 games lined — 100% `missing_pregame_stats`**.

**Root cause, confirmed live, not guessed:** `backtest_harness.get_team_stats_as_of()` strictly requires `source = 'cfbd_point_in_time'` rows in `team_game_stats`. Those rows come from `data/backfill_point_in_time_stats.py`, which calls CFBD's `/stats/season/advanced?endWeek=N` directly — but that script had **no caller anywhere in the live weekly automation** (`weekly_report.yml`, `midweek_line_pull.yml`, `post_game_audit.yml` all confirmed via grep — none reference it). It was built and has only ever been run as a manual, one-time historical backfill (2019 through last season, per its own default `--end-year`). Week 1 never hit this because week 1 always uses `get_prior_season_final_stats()` (season-1 data, which the historical backfill DID cover) — Week 2 was the first week that actually needed a real in-season point-in-time snapshot, and none existed for 2026 at all. Without a fix, **every future week of the season would have silently returned an empty card** the same way.

**Immediate unblock**: ran `python data/backfill_point_in_time_stats.py --start-year 2026 --end-year 2026 --start-week 1 --end-week 1` manually — 138 teams' Week 1 cumulative stats fetched. Card re-run: 47 of 49 lined (the other 2 have no live betting line yet — `no_line`, unrelated), all real in-season predictions, zero prior-season-fallback games, confidence split 15 `standard` / 32 `low_confidence_large_edge`, none `no_pick_extrapolation` (that flag only ever fires on prior-season-fallback input, which Week 2 correctly has none of).

**Permanent fix**: new `post_game_audit.refresh_point_in_time_stats_if_week_complete(conn, season, week)`, called from `main()`'s per-week loop right after grading each week. Gated on **zero pending live picks remaining for that week** before calling `backfill_week()` — calling it while a game is still unplayed would lock in an incomplete "through week N" snapshot, since `backfill_week()` is idempotent-by-design (skips an already-ingested week even if it was written incomplete, short of `--force`) and never re-checked automatically afterward. This ties the point-in-time refresh to the same "a week has genuinely, fully finished" moment the audit itself already has to determine (reusing §28 item 4's straggler-week machinery), so a future week's stats get fetched automatically the same run that finishes grading it — no more manual intervention required.
